# snakemake -s source_functions/smartpca.snakefile -j 1000 --rerun-incomplete --keep-going --latency-wait 30 --use-conda --resources load=100 --config --cluster-config source_functions/cluster/smartpca.cluster.json --cluster "sbatch -p {cluster.p} -o {cluster.o} --account {cluster.account} -t {cluster.t} -c {cluster.c} --mem {cluster.mem} --account {cluster.account} --mail-user {cluster.mail-user} --mail-type {cluster.mail-type} --qos {cluster.qos}" -p &> log/snakemake_log/smartpca/200812.smartpca.log
...
rule pop_labels:
	input:
		fam = lambda wildcards: expand("data/derived_data/plink_qc/thin_variants/{base_dataset}.{thin_p}/merge_thinned.{base_dataset}.{thin_p}.fam", base_dataset = config['dataset_key'][wildcards.dataset], thin_p = wildcards.thin_p),
...
rule recode_input:
	...
	# If projecting aurochs, need to use the use the same PLINK files but dataset name won't match, pull 'base_dataset' from dictionary in config
		bed = lambda wildcards: expand("data/derived_data/plink_qc/thin_variants/{base_dataset}.{thin_p}/merge_thinned.{base_dataset}.{thin_p}.bed", base_dataset = config['dataset_key'][wildcards.dataset], thin_p = wildcards.thin_p),
		bim = lambda wildcards: expand("data/derived_data/plink_qc/thin_variants/{base_dataset}.{thin_p}/merge_thinned.{base_dataset}.{thin_p}.bim", base_dataset = config['dataset_key'][wildcards.dataset], thin_p = wildcards.thin_p)
	output:
...
rule create_par:
	input:
		...
		pedsnp = "data/derived_data/smartpca/{dataset}.{thin_p}/smartpca.{dataset}.{thin_p}.pedsnp",
		pedind = "data/derived_data/smartpca/{dataset}.{thin_p}/smartpca.{dataset}.{thin_p}.pedind",
		poplist = "data/derived_data/smartpca/modern.poplist.txt"
	params:
