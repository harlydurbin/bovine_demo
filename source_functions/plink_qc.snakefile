# snakemake -s source_functions/plink_qc.snakefile -j 1000 --rerun-incomplete --keep-going --latency-wait 30 --config --cluster-config source_functions/cluster/plink_qc.cluster.json --cluster "sbatch -p {cluster.p} -o {cluster.o} --account {cluster.account} -t {cluster.t} -c {cluster.c} --mem {cluster.mem} --account {cluster.account} --mail-user {cluster.mail-user} --mail-type {cluster.mail-type}" -p &> log/snakemake_log/joint_genotyping/200812.plink_qc.log
...
rule thin_variants:
	...
	params:
		...
		out_prefix = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/thin_variants.{chr}.full.{thin_p}",
		thin_p = thin_chooser
	resources:
		load = 1
	output:
		thinned_bed = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/thin_variants.{chr}.full.{thin_p}.bed",
		thinned_bim = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/thin_variants.{chr}.full.{thin_p}.bim",
		thinned_fam = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/thin_variants.{chr}.full.{thin_p}.fam"
	shell:
		"""
		module load {params.plink_module}
...
rule merge_thinned:
	...
	params:
		plink_module = config['plink_module'],
		nt = config['plink_nt'],
		prefixes = lambda wildcards: expand("data/derived_data/plink_qc/thin_variants/full.{thin_p}/thin_variants.{chr}.full.{thin_p}\n", thin_p = wildcards.thin_p, chr = config['chr']),
		out_prefix = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/merge_thinned.full.{thin_p}"
	output:
		merge_list = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/merge_list.full.{thin_p}.list",
		merged_bed = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/merge_thinned.full.{thin_p}.bed",
		merged_bim = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/merge_thinned.full.{thin_p}.bim",
		merged_fam = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/merge_thinned.full.{thin_p}.fam"
	shell:
		"""
		module load {params.plink_module}
...
rule downsample_indiv:
	...
	params:
		plink_module = config['plink_module'],
		nt = config['plink_nt'],
		in_prefix = "data/derived_data/plink_qc/thin_variants/full.{thin_p}/merge_thinned.full.{thin_p}",
		out_prefix = "data/derived_data/plink_qc/thin_variants/{downsample_dataset}.{thin_p}/merge_thinned.{downsample_dataset}.{thin_p}"
	output:
		downsample_bed = "data/derived_data/plink_qc/thin_variants/{downsample_dataset}.{thin_p}/merge_thinned.{downsample_dataset}.{thin_p}.bed",
		downsample_bim = "data/derived_data/plink_qc/thin_variants/{downsample_dataset}.{thin_p}/merge_thinned.{downsample_dataset}.{thin_p}.bim",
		downsample_fam = "data/derived_data/plink_qc/thin_variants/{downsample_dataset}.{thin_p}/merge_thinned.{downsample_dataset}.{thin_p}.fam"
	shell:
		"""
		module load {params.plink_module}
