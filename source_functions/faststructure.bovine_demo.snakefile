...
rule structure_ml:
	input:
		structure_files = lambda wildcards: expand("data/derived_data/faststructure/structure/{dataset}.{thin_p}/structure.{dataset}.{thin_p}.k{k}.{suffix}", dataset = wildcards.dataset, thin_p = wildcards.thin_p, k = config['k'][wildcards.dataset], suffix = ["meanP", "meanQ", "varP", "varQ"])
	resources:
		load = 1
	conda:
		"envs/faststructure.yaml"
