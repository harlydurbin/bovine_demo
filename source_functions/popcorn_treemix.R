...
## Stolen from https://github.com/andrewparkermorgan/popcorn/blob/master/R/treemix.R because I can't get his package to install
...
#' 
#' @export
plot_treemix <- function(obj, text_size = 8, plot.nodes = TRUE, plot.migration = TRUE, branch.colour = "grey", label = TRUE, ...) {
  ...
  if (label)
    # Updated size to an argument
    p <- p + ggplot2::geom_text(ggplot2::aes(x = xo, y = y, label = pop),
                                hjust = 0,
                                size = text_size)
