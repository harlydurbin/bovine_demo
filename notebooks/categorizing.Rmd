...
```{r}
sorted <-  
  read_rds(here::here("data/derived_data/200605.sample_metadata.rds")) %>%
  mutate(
    region = 
      case_when(
        #### EUROPE ####
...
```{r}
sorted %>% 
  select(lab_id, population, avg_coverage, species, continent, region) %>% 
  arrange(continent, region, species, population) %>% 
  write_csv(here::here("20515.sample_metadata.csv"), na = "")
```

```{r}
sorted %>% 
  write_rds(here::here("data/derived_data/sorted.rds"))
```
