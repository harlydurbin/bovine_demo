...
read_vec("project5cattle.onepercent") %>%
  ...
  mutate(color_var = forcats::fct_inorder(color_var)) %>% 
  ggplot(aes(x = PC1,
             y = PC2,
             color = color_var,
             shape = species,
...
ggsave(here::here("figures/smartpca/project5cattle_asiansteppe.onepercent.PC1_PC2.png"), width = 7, height = 5)
```
...
* Siberian ancient clusters with Asian steppe cattle
...
* No idea what the deal with weird 3 Jinchuan yaks. They don't look especially different in fastSTRUCTURE results