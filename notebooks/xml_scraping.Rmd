...
processed <-
  read_excel(
    here::here("data/raw_data/sra_metadata/190221_Unknown_SRA.xlsx"),
    na = c("na", "NA", "missing"),
    trim_ws = TRUE
  ) %>%
  ...
  bind_rows(
    c(
      "data/raw_data/sra_metadata/200511.processed_genomes.txt",
      "data/raw_data/sra_metadata/200511.processed_genomes.outgroup.txt"
    ) %>%
      ...
      full_join(read_table2(
        here::here("data/raw_data/sra_metadata/181011_WGS.txt")
      ) %>%
...
```{r}
xmllist <-
  c(list.files(here::here("data/raw_data/sra_metadata/unknown_xml"),
             full.names = TRUE),
    here::here("data/raw_data/sra_metadata/biosample_result_full.xml")
    ) %>% 
...
```{r, eval=FALSE}
write_rds(flattened, here::here("data/derived_data/flattened_raw.rds"))
```

# Tidy `flattened`

```{r}
flattened <- read_rds(here::here("data/derived_data/flattened_raw.rds"))
```
...
flattened %<>%
  coalesce_join(
    read_excel(
      here::here(
        "data/raw_data/sra_metadata/yak_12864_2016_2702_MOESM1_ESM.xls"
      )
    ) %>%
      select(
...
```{r}
write_rds(joined, path = here::here("data/derived_data/200605.sample_metadata.rds"))

joined <- read_rds(here::here("data/derived_data/200605.sample_metadata.rds"))
```
