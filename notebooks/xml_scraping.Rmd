...
* Need to remove information for samples that I now have metadata for but didn't make it into genotyping
* Need to create a list of samples to be removed (genotyped, but since found out they're bad)

```{r}
genotyped <- 
  read_table2(here::here("data/derived_data/joint_genotyping/filter_eval/filter_eval.with_indels.28.idepth")) %>% 
  pull(1)
```
  ...
  mutate(
    international_id = 
      case_when(
        ...
        lab_id == 118017 ~ "A2494_Yenisei_River", 
        lab_id == 91166 ~ "AANUSAU000006639770", 
        lab_id == 91171 ~ "RANUSAM000000802442", 
        lab_id == 118766 ~ "UMCUSAF000000118766",
        TRUE ~ international_id
      )
  ) %>% 
  filter(international_id %in% genotyped) 
```
