...
* Rename samples from international_id --> lab_id using `bcftools reheader`

```{r,eval=FALSE}
usage_summary(process = "joint_genotyping", keyword = "reheader") 
```

```{r, eval=FALSE}
usage_facets(process = "joint_genotyping", keyword = "reheader")
```
