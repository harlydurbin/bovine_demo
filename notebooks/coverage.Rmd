...
```{r, message = FALSE, warning = FALSE, eval=FALSE}
cov_detail <- read_rds(here::here("data/derived_data/coverage/cov_detail.rds"))
```
...
cov_detail %>% 
  ...
  ungroup() %>% 
  right_join(sorted) %>% 
  select(lab_id, international_id, sex, bio_sample, population:species, avg_coverage, owner, geo_data, metadata)
```
