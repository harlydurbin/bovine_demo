...
```{r}
gq <-
  read_table2(here::here("data/derived_data/joint_genotyping/filter_eval/filter_eval.bovine_demo.28_random.GQ.FORMAT"),
              na = ".") %>% 
  tidyr::pivot_longer(cols = all_of(3:2766),
                      names_to = "international_id",
                      values_to = "gq") %>% 
  filter(international_id %in% sample_metadata$international_id) %>% 
  mutate(dataset = "bovine_demo") %>% 
  bind_rows(
    read_table2(here::here("data/derived_data/joint_genotyping/filter_eval/filter_eval.1kbulls_tranche100.28_random.GQ.FORMAT"),
                na = ".") %>% 
      tidyr::pivot_longer(cols = all_of(3:4933),
                          names_to = "international_id",
                          values_to = "gq") %>% 
      mutate(dataset = "1kbulls")) %>% 
  janitor::clean_names() %>% 
  select(-chrom)
```
...
Of 5,000 randomly pulled sites on chr 28, `r gqfail_demo %>% scales::comma(.)` where > 10% of the samples would fail/have GQ < 20 for bovine demo dataset and `r gqfail_1k %>% scales::comma(.)` where > 10% of the samples would fail/have GQ < 20 for 1K Bulls tranche100 dataset
