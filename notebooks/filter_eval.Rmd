...
```{r}
eval_28 <-
  c("1kbulls_tranche100", "bovine_demo") %>%
  purrr::set_names("1kbulls_tranche100", "demo_pre") %>%
  purrr::map_dfr(
...
```{r}
gq <-
  c("1kbulls_tranche100", "1kbulls_tranche90", "bovine_demo", "post") %>%
  purrr::set_names("1kbulls_tranche100", "1kbulls_tranche90", "demo_pre", "demo_post") %>%
  purrr::map2_dfr(
    .y = c(4933, 2766),
...
Of 5,000 randomly pulled sites on chr 28, `r gqfail_demo %>% scales::comma(.)` where > 10% of the samples would fail/have GQ < 20 for bovine demo dataset
